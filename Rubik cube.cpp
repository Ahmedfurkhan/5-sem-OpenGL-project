...
#define REP(i, n) for(std::vector<char>::size_type i = 0; i < n; ++i)
#define ll long long
...
public:
	...
	void draw() {
		...
		glPushMatrix();
		glTranslatef( (GLfloat)(x + xoff + (w - text.size() * 0.135) / 2), (GLfloat)(y + yoff - (h + 0.12) / 2),(GLfloat)(1.1));
	...
	void animate(void actionEvent()) {
		...
		if (isAnimating) {
			...
			xoff = static_cast<double>(max(0, static_cast<int>(xoff - xelev / clickdelay)));
            yoff = static_cast<double>(max(0, static_cast<int>(yoff - yelev / clickdelay)));
		}
	}

};

double button::xelev = 0.07;
double button::yelev = 0.05;
float button::xshadowMul = 0.7;
float button::yshadowMul = 0.5;
float button::clickdelay = max(10, static_cast<int>(1000 / (CUBE_SIZE * sqrt(CUBE_SIZE))));
...
double transitionSpeed = min(1.0, static_cast<double>(pow(10, floor((CUBE_SIZE - 3) / 10))) / 1000);
	...
	for (double z = start; z > -start; z -= small_size + intercube_spacing) {
		for (double y = start; y > -start; y -= small_size + intercube_spacing) {
			for (double x = -start; x < start; x += small_size + intercube_spacing) {
				...
				glTranslatef(-8, 0, 0);
				float v[8][3] = {
    { static_cast<float>(x + 8), static_cast<float>(y), static_cast<float>(z) },
    { static_cast<float>(x + small_size + 8), static_cast<float>(y), static_cast<float>(z) },
    { static_cast<float>(x + small_size + 8), static_cast<float>(y - small_size), static_cast<float>(z) },
    { static_cast<float>(x + 8), static_cast<float>(y - small_size), static_cast<float>(z) },
    { static_cast<float>(x + 8), static_cast<float>(y), static_cast<float>(z - small_size) },
    { static_cast<float>(x + small_size + 8), static_cast<float>(y), static_cast<float>(z - small_size) },
    { static_cast<float>(x + small_size + 8), static_cast<float>(y - small_size), static_cast<float>(z - small_size) },
    { static_cast<float>(x + 8), static_cast<float>(y - small_size), static_cast<float>(z - small_size) }
};
...
void displayAbout() {
	...
	printText(-3.8, 3.7, "  AHMED FURKHAN", 1.2, GLUT_STROKE_MONO_ROMAN, colorList[4], 2);
